...
export function BurnupChart({
  ...
  if (hasProjection) {
    const anchorIndexTmp = proj.fromSprintIndex as number;
    const anchorValueTmp = proj.fromDone as number;
    const vCentral = proj.projectedDonePerSprint as number;
    const vFast = vCentral * 1.2;
    const vSlow = vCentral * 0.8;
    const remainingToExtended = Math.max(0, extendedScope - anchorValueTmp);
    ...
    centralEndIdx = anchorIndexTmp + deltaIdxCentral;
    fastEndIdx = anchorIndexTmp + deltaIdxFast;
    slowEndIdx = anchorIndexTmp + deltaIdxSlow;
  }
  ...
  // for label on first open / next sprint
  let firstOpenScopeLabelX: number | null = null;
  let firstOpenScopeLabelY: number | null = null;
  let firstOpenScopeLabelValue: number | null = null;
  ...
  for (const s of allSprints) {
    ...
    if (
      ...
      !s.isClosed &&
      (lastClosed ? s.index > lastClosedIndex : true)
    ) {
      firstOpenScopeLabelX = x;
      firstOpenScopeLabelY = y;
      firstOpenScopeLabelValue = scopeValue;
    }
  }
  ...
  return (
    ...
    >
      ...
      {/* Number for first open / next sprint (slider sprint) – NO dot */}
      {firstOpenScopeLabelX != null &&
        firstOpenScopeLabelY != null &&
        firstOpenScopeLabelValue != null && (
          <text
            x={firstOpenScopeLabelX}
            y={firstOpenScopeLabelY - 8}
            fontSize={10}
            textAnchor="middle"
            fill="#16a34a"
            fontWeight={600}
          >
            {firstOpenScopeLabelValue}
          </text>
        )}
